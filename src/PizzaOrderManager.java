...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!fileIn.atEOS()) {
            String itemPricePair = fileIn.nextLine();
            int seperatorIndex = 0;

            while (itemPricePair.charAt(seperatorIndex) != ',') {
                seperatorIndex++;
            }
            priceMap.add(itemPricePair.substring(0, seperatorIndex), Integer
                    .parseInt(itemPricePair.substring(seperatorIndex + 1)));
        }
        fileIn.close();
    }
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        int price = sizePriceMap.value(input.nextLine());
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        String priceInCents = Integer.toString(price);

        output.print(priceInCents.substring(0, priceInCents.length() - 2) + "."
                + priceInCents.substring(priceInCents.length() - 2));
    }
